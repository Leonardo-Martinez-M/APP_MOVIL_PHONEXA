...
const httpClient = axios.create({
  baseURL: 'https://backend-phonexa.onrender.com',
  timeout: 10000,
  headers: {
...
httpClient.interceptors.response.use(
  ...
  async error => {
    ...
    if (error.response?.status === 401) {
      console.log('🔐 Token expirado o inválido, limpiando credenciales...');
      try {
        await Keychain.resetGenericPassword();
        console.log('✅ Credenciales limpiadas');
      } catch (keychainError) {
        console.error('Error limpiando keychain:', keychainError);
      }
    }
